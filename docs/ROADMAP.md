...
collection attempts, gaps, evidence, and principals. Complete role collections are projected
as versioned observed nodes in Neo4j, verified by deterministic digest, and promoted through
the snapshot lifecycle. Bearer-authenticated APIs and a minimal dashboard expose collection
state and active role inventory. Broader AWS entities and graph relationships remain before
M1 is complete.
