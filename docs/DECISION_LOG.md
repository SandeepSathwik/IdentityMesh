...
| 2026-09-18 | The first graph schema projects versioned observed AWS role nodes without trust or access edges | Preserve fact/inference separation until a validated attack-path rule requires relationships | Project owner | Extend through an ADR-backed relationship vocabulary in Slice 2 |
| 2026-09-18 | The local v1 data API uses one configured bearer token and an AWS account allowlist | Provide a fail-closed authentication baseline without introducing user/session infrastructure prematurely | Project owner | Replace before non-local or multi-user deployment |
